		...
		<tr>
			<td colspan="2">
				<asp:GridView ID="gvTag" runat="server" AutoGenerateColumns="False" Font-Names="Tahoma" Width="100%" SkinID="gvSkin" DataKeyNames="ID" OnRowCommand="gvTag_RowCommand" OnRowDataBound="gvTag_RowDataBound">
					...
					<Columns>
						<asp:TemplateField ItemStyle-Width="35%" HeaderText="标签名称">
							<ItemTemplate>
								<a href='ShowTags.aspx?TagID=<%# Eval("ID") %>'><%# Eval("Name")%></a> 
							</ItemTemplate>
						</asp:TemplateField>
						<asp:BoundField DataField="CatalogName"  ItemStyle-Width="16%" HeaderText="所属标签分类"/>
						<asp:BoundField DataField="ViewCount"  ItemStyle-Width="12%" HeaderText="浏览次数"/>
						<asp:TemplateField ItemStyle-Width="12%" HeaderText="公开程度">
							<ItemTemplate>
								<%# FormatFlag((byte)Eval("Flag"))%>
							</ItemTemplate>
						</asp:TemplateField>
						<asp:BoundField DataField="CreateDate"  ItemStyle-Width="15%" DataFormatString="{0:d}" HeaderText="创建时间" HtmlEncode="false"/>
						<asp:TemplateField HeaderText="操作" ItemStyle-Width="10%">
							<ItemTemplate>
								<asp:ImageButton ID="ibtMove" runat="server" CommandName="move" ImageUrl="../App_Themes/Web2ASPNET2/Images/url.gif" AlternateText="移动该标签" CommandArgument='<%# Eval("ID") %>' />&nbsp;&nbsp;
